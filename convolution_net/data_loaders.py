...
class AcousticFlatSpotDataset(Dataset):
    ...
    def __init__(self, phase, transforms=None):
        ...
        self.samples = np.load(f'{data_path}_samples.npy', mmap_mode='r')
        self.targets = np.load(f'{data_path}_targets.npy', mmap_mode='r')
...
def fetch_dataloaders(batch_size, num_workers, train_tfs, validation_tfs):
    dl_args = {'batch_size': batch_size, 'num_workers': num_workers, 'pin_memory': True}

    train_set = AcousticFlatSpotDataset('train', transforms=train_tfs)
    validation_set = AcousticFlatSpotDataset('validation', transforms=validation_tfs)
    test_set = AcousticFlatSpotDataset('test', transforms=validation_tfs)
